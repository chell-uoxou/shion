...
export function CreateFriendDialog({ open, onOpenChange, user }: CreateFriendDialogProps) {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <DialogContent className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-sm bg-[var(--brand-violet-1)] h-[500px] p-4">
                ...
                {/* 長方形入力 1 */}
                <textarea
                    className="w-full h-15 rounded-xl border border-gray-300 p-4 mt-4 resize-none"
                    placeholder="名前を入力…"
                    value={text1}
